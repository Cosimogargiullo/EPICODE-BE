...
import it.epicode.bw.models.TratteEnum;
...
//		 ISTANZA MEZZO
		MezzoTrasporto m1 = new MezzoTrasporto(StatoMezzo.InSERVIZIO, null, null, LocalDate.now(),
				LocalDate.now().plusDays(1), 0, TipoMezzo.AUTOBUS, null);
...
//		ISTANZA TRATTA
		Tratta tr1 = new Tratta(TratteEnum.BATTISTINI_ANAGNINA);
		Tratta tr2 = new Tratta(TratteEnum.SANGIOVANNI_PANTANO);
		ParcoMezziDAO.save(tr1);
		ParcoMezziDAO.save(tr2);
		
//		ASSEGNAZIONE TRATTA
		ParcoMezziDAO.assegnaTratta(m1, tr1);
		ParcoMezziDAO.percorriTratta(m1, 23);
		ParcoMezziDAO.percorriTratta(m1, tr2, 24);
		
		System.out.println(m1.getTrattePercorse());
		...
		int scelta;
		do {
			System.out.printf("Scegli operazione : %n" + "1 - Crea nuova tessera %n" + "2 - Acquista un biglietto %n"
					+ "3 - Verifica biglietti venduti %n" + "4 - Verifica biglietti venduti per periodo %n"
					+ "5 - Verifica validita abbonamento %n" + "6 - Rinnova tessera %n");
